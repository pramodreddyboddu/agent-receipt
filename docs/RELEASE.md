...
cannot push `.github/workflows/*`. v1.0.6 through v1.0.22 left
...
containing PROVED and the receipt sha256), and a 1.0.22 config `sign: true`
smoke (bare `wrap` writes `*.sig.json`, `--no-sign` does not, and missing
keys tip and leave the receipt unsigned).
The drop-in PR gate
