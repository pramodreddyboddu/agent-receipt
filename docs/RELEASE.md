...
cannot push `.github/workflows/*`. v1.0.6 through v1.0.12 left
...
`doctor --json`, `audit --event wrap`, `audit --agent ci --failed`, and
`history --agent ci` / `history --uncommitted` / `history --failed`.
