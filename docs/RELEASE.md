...
cannot push `.github/workflows/*`. v1.0.6 through v1.0.17 left
...
`sign` / `prove --json` signature smoke and a 1.0.17 `verify --require-sig`
check (exit 0 signed, exit 2 unsigned). The drop-in PR gate
