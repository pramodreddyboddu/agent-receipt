...
function initRepo(prefix) {
  ...
  writeFileSync(join(dir, 'README.md'), '# v1.0.8 fixture\n');
...
describe('prune audit events', () => {
  ...
  it('dry-run plans identity fields and does not append', () => {
    ...
    assert.equal(report.version, '1.0.8');
  ...
  it('apply appends one prune line per deleted receipt and keeps the chain', () => {
    ...
    assert.equal(chain.version, '1.0.8');
...
describe('doctor --strict', () => {
  ...
  it('stays exit 0 when the directory is small', () => {
    ...
    assert.match(strict.out, /Ready/);
    assert.match(cli(dir, ['help', 'doctor']), /--strict/);
  });

  it('fails unset policy and retention only under pressure, and only with --strict', () => {
